      ...
      <div className="p-4 border-b border-white/20 flex-shrink-0">
        ...
        <Card className="p-4 mb-4 glass-card">
          <div className="flex items-center justify-between mb-3">
            <h3 className="font-medium text-white drop-shadow">Audio Overview</h3>
          </div>
            ...
            /> : <Card className="p-3 glass-card">
              ...
              {currentStatus !== 'generating' && !isGenerating && !isAutoRefreshing && <div className="flex items-center space-x-3 mb-3">
                  ...
                  <div className="flex-1">
                    <h4 className="font-medium text-white drop-shadow">Deep Dive conversation</h4>
                    <p className="text-sm text-white/80">Two hosts</p>
                  </div>
                </div>}
              
              {/* Status Display */}
              {getStatusDisplay() && <div className="flex items-center space-x-2 mb-3 p-2 rounded-md glass">
                  {getStatusDisplay()!.icon}
                  <div className="flex-1">
                    <p className="text-sm font-medium text-white drop-shadow">{getStatusDisplay()!.text}</p>
                    <p className="text-xs text-white/80">{getStatusDisplay()!.description}</p>
                  </div>
                </div>}
              
              {/* Audio error div */}
              {audioError && <div className="flex items-center space-x-2 mb-3 p-2 glass-dark rounded-md">
                  <AlertCircle className="h-4 w-4 text-red-400" />
                  <div className="flex-1">
                    <p className="text-sm text-red-400">Audio unavailable</p>
                  </div>
                  <Button size="sm" variant="outline" onClick={handleAudioRetry} className="text-red-400 border-red-400/50 hover:bg-red-500/20 glass-button">
              ...
              <div className="flex space-x-2">
                <Button size="sm" onClick={handleGenerateAudio} disabled={isGenerating || currentStatus === 'generating' || !hasProcessedSource || isAutoRefreshing} className="flex-1 text-white glass-strong border-white/30 hover:bg-white/30">
        ...
        <div className="mb-4">
          <div className="flex items-center justify-between mb-3">
            <h3 className="font-medium text-white drop-shadow">Notes</h3>
            
          </div>
          
          <Button variant="outline" size="sm" className="w-full mb-4 glass-button text-white border-white/30" onClick={handleCreateNote}>
      ...
      <ScrollArea className="flex-1 h-full">
        <div className="p-4">
          {isLoading ? <div className="text-center py-8">
              <p className="text-sm text-white/80">Loading notes...</p>
            </div> : notes && notes.length > 0 ? <div className="space-y-3">
              {notes.map(note => <Card key={note.id} className="p-3 glass-card hover:bg-white/20 cursor-pointer transition-all duration-300 hover:scale-105" onClick={() => handleEditNote(note)}>
                  <div className="flex items-start justify-between">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center space-x-2 mb-1">
                        {note.source_type === 'ai_response' ? <Bot className="h-3 w-3 text-blue-400" /> : <User className="h-3 w-3 text-white/60" />}
                        <span className="text-xs text-white/70 uppercase">
                          {note.source_type === 'ai_response' ? 'AI Response' : 'Note'}
                        </span>
                      </div>
                      <h4 className="font-medium text-white truncate drop-shadow">{note.title}</h4>
                    ...
                    {note.source_type === 'user' && <Button variant="ghost" size="sm" className="ml-2 glass-button text-white border-white/30">
              ...
              <h3 className="text-lg font-medium text-white mb-2 drop-shadow">Saved notes will appear here</h3>
